function addShortTermData(ticker, callback) {
    fetch(`https://www.sawusdomain.com/Sites/OnlineBroker/PHP/shortTermScript.php?ticker=${ticker}`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            const dataBox = document.getElementById('short-term-data-box');
            const averageBox = document.getElementById('short-term-average-value'); // Target the new span element
